source("/Users/bhavnasharma/Downloads/cooperative_learning_multi_view.R")


# Define the file paths
tsv_files <- list(
  MIR = "/Users/bhavnasharma/Downloads/mir.tsv",
  METH = "/Users/bhavnasharma/Downloads/meth.tsv",
  RNA = "/Users/bhavnasharma/Downloads/rna.tsv"
)

survival_tsv <- "/Users/bhavnasharma/Downloads/survival.tsv"

tsv_test <- list(
  RNA = "/Users/bhavnasharma/Downloads/rna.tsv"
)

test_survival <- "/Users/bhavnasharma/Downloads/survival.tsv"



# Load the data from the tsv_files
MIR_data <- read.table(tsv_files$MIR, header = TRUE, sep = "\t")
METH_data <- read.table(tsv_files$METH, header = TRUE, sep = "\t")
RNA_data <- read.table(tsv_files$RNA, header = TRUE, sep = "\t")

# Load the survival data
survival_data <- read.table(survival_tsv, header = TRUE, sep = "\t")

rna_samples <- RNA_data$Samples

# Subset y to only include rows where 'samples' value is in rna_samples
survival_data <- survival_data[survival_data$Samples %in% rna_samples, ]
...
for (ii in 1:simN){
  ...
  # Update the dimensionality according to the new data
  px1 = ncol(x1)
  px2 = ncol(x2)
  px3 = ncol(x3)
